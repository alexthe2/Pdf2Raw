#include "Interpret.h"
Interpret::Interpret(const cv::Mat img, const std::string tess_location) {
	this->img_ = img;
...
Interpret::Interpret(const ImageSelection is, const std::string tess_location) {
...
auto Interpret::InitAndEvaluate() -> void {
	...
	this->ocr_->Init(this->tess_location_.c_str(), "eng", tesseract::OEM_LSTM_ONLY);
